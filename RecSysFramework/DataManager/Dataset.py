...
class Dataset(object):
    ...
    def add_dataset(self, new_dataset, add_URM=True, add_ICM=True, add_UCM=True):

        if add_URM:
            for URM_name in self.get_URM_names():
                self.URM_dict[URM_name] += new_dataset.get_URM(URM_name)

        if add_ICM:
            for ICM_name in self.get_ICM_names():
                self.ICM_dict[ICM_name] += new_dataset.get_ICM(ICM_name)

        if add_UCM:
            for UCM_name in self.get_UCM_names():
                self.UCM_dict[UCM_name] += new_dataset.get_UCM(UCM_name)
