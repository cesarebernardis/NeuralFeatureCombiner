    ...
    def _loadICM(self, tags_path, item_mapper, feature_columns=None, filter_and_stem=False, null_character="\\N",
        ...
        for line in fileHandle:
            ...
            if len(line) > 1:
                ...
                if line[11] != null_character:
                    names = [s.lower().strip() for s in line[11].split("|") if len(s) > 0]
                else:
                    names = []

                # List of director ids
                if line[12] != null_character:
                    ids = [int(s) for s in line[12].split("|") if len(s) > 0]
                    features.append(ids)
                    if len(names) == len(ids):
                        directors_name_to_id.update(dict(zip(names, ids)))
                else:
                    features.append(None)
                ...
                # List of actor names
                if line[16] != null_character:
                    names = [s.lower().strip() for s in line[16].split("|") if len(s) > 0]
                else:
                    names = []

                # List of actor ids
                if line[17] != null_character:
                    ids = [int(s) for s in line[17].split("|") if len(s) > 0]
                    features.append(ids)
                    if len(names) == len(ids):
                        actors_name_to_id.update(dict(zip(names, ids)))
                else:
                    features.append(None)
        ...
        zipFile_path = self.DATASET_SPLIT_ROOT_FOLDER + self.DATASET_SUBFOLDER + os.sep

        import pickle

        os.makedirs(zipFile_path, exist_ok=True)
        with open(zipFile_path + "directors_name_to_id.pkl", "wb") as file:
            pickle.dump(directors_name_to_id, file)
        with open(zipFile_path + "actors_name_to_id.pkl", "wb") as file:
            pickle.dump(actors_name_to_id, file)
...
class YahooMoviesReducedReader(YahooMoviesReader):

    DATASET_SUBFOLDER = "YahooMoviesReduced/"
    ADD_SYNOPSIS = False
