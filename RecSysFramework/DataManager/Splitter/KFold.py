...
class KFold(DataSplitter):
    ...
    def __init__(self, n_folds=5, allow_cold_users=False, test_rating_threshold=0):
        """
        ...
        super(KFold, self).__init__(allow_cold_users=allow_cold_users, with_validation=False)
...
class ColdItemsKFold(KFold):
    ...
    def _get_balanced_split(self, URM, rounds=10):
        ...
        expected_interactions = len(URM.data) / self.n_folds
        best_error = np.inf

        for _ in range(rounds):

            new_items_split = np.random.choice(self.n_folds, n_items, replace=True)
            error = sum(
                (expected_interactions - itempop[items[new_items_split == i]].sum()) ** 2
                for i in range(self.n_folds)
            )

            if error < best_error:
                best_error = error
                items_split = new_items_split
    ...
    def split(self, dataset, random_seed=42):
        ...
        folds = []
        split_belonging = self._get_balanced_split(dataset.get_URM())
