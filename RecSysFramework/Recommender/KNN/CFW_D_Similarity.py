...
class CFW_D(BaseItemCBFRecommender, ItemSimilarityMatrixRecommender, EarlyStoppingModel):
    ...
    def __init__(self, URM_train, ICM, S_matrix_target):
        ...
        super(CFW_D, self).__init__(URM_train, ICM)
    ...
    def generateTrainData_low_ram(self):
        ...
        S_matrix_contentKNN = sim.dot_product(self.ICM_train, self.ICM_train.T, k=self.topK).tocsr()
        ...
        if self.normalize_similarity:
            ...
            sum_of_squared_features = np.array(self.ICM_train.T.power(2).sum(axis=0)).ravel()
    ...
    def get_embedded_ICM(self, use_incremental=False):
        if use_incremental:
            D = self.D_incremental
        else:
            D = self.D_best
        return self.ICM_train * sps.diags(D)


    def get_embedding(self, features, use_incremental=False):
        if use_incremental:
            D = self.D_incremental
        else:
            D = self.D_best
        return sps.csr_matrix(features).multiply(D)
    ...
    def compute_W_sparse(self, use_incremental=False):
        ...
        self.W_sparse = sim.dot_product(self.ICM_train * sps.diags(D), self.ICM_train.T, k=self.topK).tocsr()
        self.sparse_weights = True


    def set_ICM_and_recompute_W(self, ICM_new, recompute_w=True):

        self.ICM_train = ICM_new.copy()
    ...
    def fit(self, logFile=None, precompute_common_features=True,
        ...
        self.cythonEpoch = CFW_D_Similarity_Cython_SGD(self.row_list, self.col_list, self.data_list,
                                                      self.n_features, self.ICM_train,
        ...
        self.D_incremental = self.cythonEpoch.get_D()
