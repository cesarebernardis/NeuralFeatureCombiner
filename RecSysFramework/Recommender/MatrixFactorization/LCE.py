...
class LCE(Recommender):
    ...
    def fit(self, k=500, alpha=0.5, beta=0.05, lamb=0.5,
        ...
        print("{}: Matrix A contructed".format(self.RECOMMENDER_NAME))
        ...
        print("{}: Preprocessing completed, starting iterations".format(self.RECOMMENDER_NAME))
        ...
        while delta > epsilon and itNum <= maxiter:
            ...
            if itNum > 1:
                ...
                if verbose:
                    print('{}: Iteration: {} \t Objective: {:.5f} \t Delta: {:.5f}'
                          .format(self.RECOMMENDER_NAME, itNum, Obj, delta))
            elif verbose:
                print('{}: Iteration: {} \t Objective: {:.5f}'
    ...
    def _compute_item_score(self, user_id_array, items_to_compute=None):
        ...
        assert self.Hu.shape[1] > user_id_array.max(),\
                "LCE: Cold users not allowed. Users in trained model are {}, requested prediction for users up to {}"\
                    .format(self.Hu.shape[1], user_id_array.max())

        if items_to_compute is not None:
            item_scores = - np.ones((len(user_id_array), self.URM_train.shape[1]), dtype=np.float32) * np.inf
            item_scores[:, items_to_compute] = self.Ws_test.dot(self.Hu[:, user_id_array])[:, items_to_compute]
        else:
    ...
    def get_embedded_ICM(self):
        return self.Ws_test.copy()


    def get_embedding(self, features):
        return lstsq(self.Hs.T, sps.csr_matrix(features).toarray().T, rcond=None)[0].T
