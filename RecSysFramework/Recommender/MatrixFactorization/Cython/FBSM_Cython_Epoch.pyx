...
cdef class FBSM_Cython_Epoch:
    ...
    def __init__(self, URM, ICM, n_factors=1,
                 epochs=30, precompute_user_feature_count=False,
                 learning_rate=0.05, l2_reg_D=0.0, l2_reg_V=0.0,
                 sgd_mode='adagrad', weights_initialization=None,
        ...
        # self.D = np.random.normal(mean_init, std_init, (self.n_features)).astype(np.float64)
        self.V = np.random.normal(mean_init, std_init, (self.n_factors, self.n_features)).astype(np.float64)

        #self.V = np.zeros((self.n_factors, self.n_features)).astype(np.float64)
    ...
    def epochIteration_Cython(self):
        ...
        cdef double cum_loss=0.0, r_uij_hat, sigma_deriv, loss, loss_deriv, learning_rate_current_feature
        ...
        for num_sample in range(self.n_users):
            ...
            if (num_sample % 500000 == 0 and num_sample!=0) or num_sample == self.n_samples-1:
                print ("Processed {} out of {} samples ( {:.2f}%). BPR loss is {:.4E}. Sample per second {:.0f}".format(
            ...
            #print("get f, delta, userF and precompute: {}".format(time.time()-start_time))

            # Add D component
            r_uij_hat = 0.0

            # Ddelta_ij = D (1,#Features) dot delta_ij (#Features,1)
            # Ddelta_ij = clone(template_zero, self.n_features, zero=True)
            ...
            # V_f_u is (#factors,1)
            # V_f_u = clone(template_zero, self.n_factors, zero=True)
            for factor_index in range(self.n_factors):
            ...
            # Vfi (#factors,1)
            # Vfi = clone(template_zero, self.n_factors, zero=True)
            for factor_index in range(self.n_factors):
            ...
            if updated_features_count > 0:
                ...
                for feature_index in range(updated_features_count):
                    ...
                    if self.useAdaGrad:
                        self.sgd_cache_D[feature_id] += loss_deriv ** 2
                        learning_rate_current_feature = self.learning_rate / (sqrt(self.sgd_cache_D[feature_id]) + 1e-8)

                        self.D[feature_id] += learning_rate_current_feature * (loss_deriv - 2 * self.l2_reg_D * self.D[feature_id])

                    elif self.useRmsprop:
                        self.sgd_cache_D[feature_id] = self.sgd_cache_D[feature_id] * self.gamma + (1 - self.gamma) * loss_deriv ** 2
                        learning_rate_current_feature = self.learning_rate / (sqrt(self.sgd_cache_D[feature_id]) + 1e-8)
                        ...
                        self.sgd_cache_D_momentum_1[feature_id] = \
                            self.sgd_cache_D_momentum_1[feature_id] * self.beta_1 + (1 - self.beta_1) * loss_deriv

                        self.sgd_cache_D_momentum_2[feature_id] = \
                            self.sgd_cache_D_momentum_2[feature_id] * self.beta_2 + (1 - self.beta_2) * loss_deriv**2
                        ...
                        loss_deriv = self.momentum_1/ (sqrt(self.momentum_2) + 1e-8)

                        self.D[feature_id] += self.learning_rate * (loss_deriv - 2 * self.l2_reg_D * self.D[feature_id])
