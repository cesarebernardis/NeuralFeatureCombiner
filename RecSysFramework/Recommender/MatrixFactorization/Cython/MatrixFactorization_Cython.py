...
class BPRMF_AFM(_MatrixFactorization_Cython):
    ...
    def get_embedding(self, features):
        return features[:, self.features_to_keep] * self.cythonEpoch.get_ITEM_factors()


    def fit(self, epochs=300, num_factors=10, positive_threshold_BPR=None,
            learning_rate=0.01, sgd_mode='sgd', user_reg=0.0, feature_reg=0.0,
            init_mean=0.0, init_std_dev=0.1, stop_on_validation=False, lower_validations_allowed=None,
            validation_metric="MAP", evaluator_object=None, validation_every_n=None):

        self.use_bias = False
        self.negative_interactions_quota = False
        self.num_factors = num_factors
        self.sgd_mode = sgd_mode
        self.batch_size = 1
        self.positive_threshold_BPR = positive_threshold_BPR
        ...
        from .BPRMF_AFM_Cython_epoch import BPR_AFM_Cython_Epoch
