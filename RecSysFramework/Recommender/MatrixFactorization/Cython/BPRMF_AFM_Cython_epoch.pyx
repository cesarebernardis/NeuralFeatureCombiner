...
cdef class BPR_AFM_Cython_Epoch:
    ...
    def __init__(self, URM_train, ICM, n_factors=10,
        ...
        self.loss = 0.0
        self.USER_factors = np.random.normal(init_mean, init_std_dev, (self.n_users, self.n_factors)).astype(np.float64)
        self.ITEM_factors = np.random.normal(init_mean, init_std_dev, (self.n_features, self.n_factors)).astype(np.float64)
