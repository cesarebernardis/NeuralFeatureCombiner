...
cdef class HP3_Similarity_Cython_SGD:
    ...
    cdef float[:] icm_data, commonFeatures_data, common_features_data, common_features_data_i
    ...
    def __init__(self, row_list, col_list, data_list, n_features, ICM, simplify_model=True,
                 precompute_common_features=True, weights_initialization=None,
                 use_dropout=False, dropout_perc=0.3, learning_rate=0.05, l1_reg=0.0, l2_reg=0.0,
        ...
        self.data_list = np.array(data_list, dtype=np.float32)
        ...
        self.common_features_data = np.zeros((self.n_features), dtype=np.float32)
        self.common_features_data_i = np.zeros((self.n_features), dtype=np.float32)
        ...
        self.icm_data = np.array(ICM.data, dtype=np.float32)
        ...
        dpf = np.array(ICM.sum(axis=0).astype(np.float64)).flatten()
        dpf[dpf != 0] = np.reciprocal(dpf[dpf != 0])
        self.dpfInverse = dpf
    ...
    def precompute_common_features_function(self, ICM):
        ...
        self.commonFeatures_data = commonFeatures.data.astype(np.float32)
    ...
    def epochIteration_Cython(self):
        ...
        cdef float[:] f_row_data, f_col_data
        ...
        # Shuffle data
        cdef long[:] newOrdering = np.random.permutation(n_samples)
        ...
        for sample_num in range(n_samples):
            ...
            for feature_index in range(num_common_features):
                ...
                numerator += self.dpfInverse[feature_id] * self.weights[feature_id]

            denominator = 0.0
            for k in range(len(f_row)):
                denominator += self.weights[f_row[k]]
            ...
            for feature_index in range(len(f_row)):
                ...
                if self.dropout_mask[feature_id]:
                    ...
                    if self.useAdaGrad:
                        ...
                        learning_rate_current_feature = self.learning_rate / (sqrt(self.sgd_cache_D[feature_id]) + 1e-20)

                        self.weights[feature_id] -= learning_rate_current_feature * (error + self.l1_reg + 2*self.l2_reg * self.weights[feature_id])
                        ...
                        gradient_update = self.momentum_1/ (sqrt(self.momentum_2) + 1e-20)

                        self.weights[feature_id] -= self.learning_rate * (gradient_update + self.l1_reg + 2*self.l2_reg * self.weights[feature_id])

                    else:

                        self.weights[feature_id] -= self.learning_rate * (error + self.l1_reg + 2*self.l2_reg * self.weights[feature_id])

                    # Clamp weight if needed
                    if self.weights[feature_id] <= 0.0:
                        self.weights[feature_id] = 1e-20
    ...
    def get_weights(self):
        return np.array(self.weights).flatten().copy()
    ...
    cdef float[:] get_features_vector_data(self, long index):
        return self.icm_data[self.icm_indptr[index]:self.icm_indptr[index + 1]]


    cdef int feature_common_unordered(self, int[:] feature_i, int[:] feature_j, float[:] data_i, float[:] data_j):
    ...
    def feature_common_test(self):
        ...
        count = self.feature_common_unordered(f_i, f_j, np.ones_like(f_i, dtype=np.float32), np.ones_like(f_j, dtype=np.float32))
