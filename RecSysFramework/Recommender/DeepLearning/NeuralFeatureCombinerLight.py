...
class NeuralFeatureCombinerLight(BaseItemCBFRecommender, ItemSimilarityMatrixRecommender, EarlyStoppingModel):

    RECOMMENDER_NAME = "NeuralFeatureCombinerLight"

    def __init__(self, URM_train, ICM, S_matrix_target):
        super(NeuralFeatureCombinerLight, self).__init__(URM_train, ICM)
        self.S_matrix_target = S_matrix_target.tocsr(copy=True) / S_matrix_target.max()
        self.S_matrix_target = self.S_matrix_target.astype(np.float32)
        self.graph = None
        self.session = None
        self.embedded_ICM = None


    def build_model(self, cb_items, content_input, collaborative_input, target):
        ...
        with self.graph.as_default():
            ...
            batchsize = tf.size(input=cb_items)

            with tf.compat.v1.name_scope("content_dnn"):

                content_input = tf.sparse.SparseTensor(content_input[0], content_input[1],
                                                       (batchsize, self.n_features))

                output = content_input
            ...
            with tf.compat.v1.name_scope("collaborative_dnn"):
                ...
                output = tf.sparse.SparseTensor(collaborative_input[0], collaborative_input[1],
                                                            (batchsize, self.n_users))
            ...
            with tf.compat.v1.name_scope("similarity"):
                self.output = tf.keras.backend.batch_dot(self.content_embedding, self.collaborative_embedding)
    ...
    def fit(self, layers=None, topK=200, epochs=30, batch_size=128, l2_reg=1e-4,
        ...
        with self.graph.as_default():
            ...
            self.train_rows_ph = tf.compat.v1.placeholder(tf.int32, [None])
            self.train_cols_ph = tf.compat.v1.placeholder(tf.int32, [None])
            self.train_data_ph = tf.compat.v1.placeholder(tf.float32, [None])

            self.train_rows, self.train_cols, self.train_data = \
                urm_to_coordinate_list(self.S_matrix_target[self.train_items, :][:, self.train_items],
                                           self.add_zeros_quota, random_seed=0)
            self.URM_train = self.URM_train.tocsc()

            def gen_sample(rows, cols, data):
                features = self.ICM_train[cols, :].tocoo()
                profiles = self.URM_train[:, rows].transpose().tocoo()
                return rows, cols, \
            ...
            def tf_wrapper(rows, cols, data):
                return tf.numpy_function(func=gen_sample, inp=[rows, cols, data],
                                  Tout=[tf.int32, tf.int32, tf.int64, tf.float32, tf.int64, tf.float32, tf.float32])

            dataset = tf.data.Dataset.from_tensor_slices(
                (self.train_rows_ph, self.train_cols_ph, self.train_data_ph)
            )
            ...
            self.cf_items, self.cb_items, cb_in1, cb_in2, cf_in1, cf_in2, self.target = self.iterator.get_next()

            self.build_model(self.cb_items, (cb_in1, cb_in2), (cf_in1, cf_in2), self.target)
    ...
    def _run_epoch(self, num_epoch):
        ...
        pbar = tf.keras.utils.Progbar(len(self.train_items), width=50, verbose=1)

        with self.graph.as_default(), self.session.as_default() as session:

            session.run(self.iterator.initializer, feed_dict={
                self.batch_size_ph: self.batch_size,
                self.train_rows_ph: self.train_items[self.train_rows],
                self.train_cols_ph: self.train_items[self.train_cols],
            ...
            while True:
                try:
                    ...
                    if self.train_writer is not None:
                        output, summaries, loss, _ = session.run(
                            [self.output, self.summaries, self.loss_ph, self.training_op]
                        )
                        self.train_writer.add_summary(summaries, num_epoch)
                    else:
                        output, loss, _ = session.run([self.output, self.loss_ph, self.training_op])

                    cumloss += loss
                    pbar.add(output.shape[0], values=[('loss', loss)])
            ...
            if self.global_step_val % 3 == 0:
                self.train_rows, self.train_cols, self.train_data = \
                    urm_to_coordinate_list(self.S_matrix_target[self.train_items, :][:, self.train_items],
                                               self.add_zeros_quota, random_seed=self.global_step_val)
        ...
        with self.session.as_default():
            ...
            for item in range(self.n_items):
                ...
                self.session.run(self.iterator.initializer, feed_dict={
                    self.batch_size_ph: self.n_train_items,
                    self.train_rows_ph: self.train_items[np.arange(self.n_train_items)],
                    self.train_cols_ph: np.repeat(item, self.n_train_items),
                })

                while True:
                    try:

                        rows, data = self.session.run([self.cf_items, self.output])
                        data = data[rows] # They might be shuffled
                        mask = np.argsort(-data)[:topK]
                        data[item, :] = data[mask]
                        indices[item, :] = rows[mask]

                        pbar.add(1)
                    except tf.errors.OutOfRangeError:
                        break

        self.W_sparse = sps.csc_matrix((data.flatten(), indices.flatten(),
...
class NeuralFeatureCombinerLight_OptimizerMask(NeuralFeatureCombinerLight):

    def fit(self, topK=200, epochs=30, batch_size=128, l2_reg=1e-4, dropout=0.2, rnd_seed=42,
            activation_function="relu", add_zeros_quota=2., learning_rate=0.0001, optimizer=None,
        ...
        super(NeuralFeatureCombinerLight_OptimizerMask, self).fit(
