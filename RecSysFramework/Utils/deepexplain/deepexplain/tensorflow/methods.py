...
class AttributionMethod(object):
    ...
    def _session_run_batch(self, T, xs, ys=None):
        feed_dict = {}
        if self.fixed_feed_dict is not None:
            for k, v in self.fixed_feed_dict.items():
                feed_dict[k] = v
...
class GradientBasedMethod(AttributionMethod):
    ...
    def run(self, xs, ys=None, batch_size=None, fixed_feed_dict=None):
        self._check_input_compatibility(xs, ys, batch_size)
        self.fixed_feed_dict=fixed_feed_dict
...
class DeepExplain(object):
    ...
    def explain(self, method, T, X, xs, ys=None, batch_size=None, fixed_feed_dict=None, **kwargs):
        explainer = self.get_explainer(method, T, X, **kwargs)
        return explainer.run(xs, ys, batch_size, fixed_feed_dict)
