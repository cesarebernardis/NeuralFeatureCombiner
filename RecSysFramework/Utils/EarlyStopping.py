...
class EarlyStoppingModel(object):
    ...
    def get_early_stopping_final_epochs_dict(self):
        ...
        return {"epochs": self.epochs_best, "best_results_run": self.best_results_run,
                "best_results_evaluation_time": self.best_results_evaluation_time,
    ...
    def _train_with_early_stopping(self, epochs_max, epochs_min=0,
        ...
        while epochs_current < epochs_max and not convergence:
            ...
            # Check if the model has the method to run multiple epochs with a single call
            if callable(getattr(self, "_run_multiple_epochs", None)):
                if evaluator_object is None or validation_every_n is None:
                    validation_every_n = epochs_max
                self._run_multiple_epochs(epochs_current, validation_every_n)
                epochs_current += validation_every_n
            else:
            ...
            # Determine whether a validaton step is required
                ...
                results_run = metrics_handler.get_results_dictionary(use_metric_name=False)
            ...
            if self.loss is not None:
                loss_str = "Loss: {:.6f} ".format(self.loss)
            else:
                loss_str = ""

            print("{}: Epoch {} of {}. {}Elapsed time {:.2f} {}".format(
                algorithm_name, epochs_current, epochs_max, loss_str, new_time_value, new_time_unit))
