...
class EvaluatorMetrics(Enum):
    ...
    COVERAGE_TEST_ITEM = Coverage_Test_Item
    ...
    for cutoff in results_run.keys():
        ...
        for metric in results_run_current_cutoff.keys():
            ...
            if isinstance(val, np.ndarray):
                ...
                val = np.mean(val)
            metrics_str.append("{}: {:.{n_decimals}f}".format(metric, val, n_decimals=n_decimals))

        output_str += "CUTOFF: {} - {}\n".format(cutoff, ", ".join(metrics_str))
...
class MetricsHandler(object):
    ...
    _BASIC_METRICS = [
        ...
        EvaluatorMetrics.COVERAGE_TEST_ITEM,
    ]
